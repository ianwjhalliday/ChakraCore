...
ParseNodePtr Parser::ParseMetaProperty(tokens metaParentKeyword, charcount_t ichMin, _Out_opt_ BOOL* pfCanAssign)
{
    ...
    if (this->m_token.tk == tkID && this->m_token.GetIdentifier(m_phtbl) == this->GetTargetPid())
    {
        ...
        if (buildAST)
        {
            return m_ast.CreateNode<knopNewTarget>(ichMin);
        }
    }
    else
    {
    ...
    _Out_opt_ RestorePoint* pEllipsisLocation /*= nullptr*/)
{
    ...
    case tkTHIS:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopThis>();
        }
        fCanAssign = FALSE;
        m_pscan->Scan();
        break;
    ...
    case tkLParen:
    {
        ...
        if (m_token.tk == tkRParen)
        {
            ...
            if (buildAST)
            {
                pnode = m_ast.CreateNode<knopEmpty>();
            }
            break;
        }
    ...
    case tkIntCon:
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateIntNode(m_token.GetLong());
        }
        fCanAssign = FALSE;
        m_pscan->Scan();
        break;
    ...
    case tkFltCon:
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopFlt>();
    ...
    case tkStrCon:
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateStrNode(m_token.GetStr());
        }
        else
        {
    ...
    case tkTRUE:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopTrue>();
        }
        fCanAssign = FALSE;
        m_pscan->Scan();
        break;
    ...
    case tkFALSE:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopFalse>();
        }
        fCanAssign = FALSE;
        m_pscan->Scan();
        break;
    ...
    case tkNULL:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopNull>();
        }
        fCanAssign = FALSE;
        m_pscan->Scan();
        break;
...
LFunction :
    ...
    pnode = ParsePostfixOperators<buildAST>(pnode, fAllowCall, fInNew, &fCanAssign, &pidTermId, pfIsDotOrIndex);
...
ParseNodePtr Parser::ParseRegExp()
{
    ...
    if (buildAST || m_doingFastScan)
    {
        ...
        pnode = m_ast.CreateNode<knopRegExp>();
                ...
                *ppidTermId = nullptr; // This is no longer an identifier
                ...
                if (shouldConvertToDot)
                {
                       ...
                       !Js::TaggedInt::IsOverflow(uintValue)) // the optimization is not very useful if the number can't be represented as a TaggedInt
                    {
                        ...
                        auto intNode = m_ast.CreateIntNode(uintValue); // implicit conversion from uint32 to int32
        ...
        case tkDot:
            ...
            if (buildAST)
            {
                ...
                else
                {
                    ...
                    name = m_ast.CreateStrNode(m_token.GetIdentifier(m_phtbl));
                }
...
ParseNodePtr Parser::ParseArrayLiteral()
{
    ...
    if (buildAST)
    {
        pnode = m_ast.CreateNode<knopArray>();
    ...
    *count = 0;
    ...
    for (;;)
    {
        ...
        if (tkComma == m_token.tk || tkRBrack == m_token.tk)
        {
            ...
            if (buildAST)
            {
                pnodeArg = m_ast.CreateNode<knopEmpty>();
            }
        }
        else
        {
...
template<bool buildAST> void Parser::ParseComputedName(ParseNodePtr* ppnodeName, LPCOLESTR* ppNameHint, LPCOLESTR* ppFullNameHint, uint32 *pNameLength, uint32 *pShortNameOffset)
{
    ...
    if (buildAST)
    {
        *ppnodeName = m_ast.CreateNode<knopComputedName>(pnodeNameExpr->ichMin, pnodeNameExpr->ichLim);
    ...
    *ppNameHint=nullptr;
    ...
    case tkID:
        ...
        *ppNameHint = pid->Psz();
        if (buildAST)
        {
            pnodeName = m_ast.CreateStrNode(pid);
        }
        break;
    case tkStrCon:
        if (IsStrictMode() && m_pscan->IsOctOrLeadingZeroOnLastTKNumber())
...
ParseNodePtr Parser::ParseMemberList(LPCOLESTR pNameHint, uint32* pNameHintLength, tokens declarationType)
{
    ...
    for (;;)
    {
        ...
        case tkID:
            ...
            if (buildAST)
            {
                pnodeName = m_ast.CreateStrNode(pidHint);
            }
            break;
...
ParseNodePtr Parser::GenerateEmptyConstructor(bool extends)
{
    ...
    if (extends)
    {
        ...
        ParseNodePtr superRef = m_ast.CreateNode<knopSuper>();
    ...
    AddToNodeList(&pnodeFnc->sxFnc.pnodeBody, &lastNodeRef, m_ast.CreateNode<knopEndCode>());
...
void Parser::ParseExpressionLambdaBody(ParseNodePtr pnodeLambda)
{
    ...
    if (buildAST)
    {
        pnodeRet = m_ast.CreateNode<knopReturn>();
    ...
    if (buildAST)
    {
        ...
        ParseNodePtr end = m_ast.CreateNode<knopEndCode>(pnodeRet->ichLim);
...
ParseNodePtr Parser::ParseClassDecl(BOOL isDeclaration, LPCOLESTR pNameHint, uint32 *pHintLength, uint32 *pShortNameOffset)
{
    ...
    for (;;)
    {
        ...
        {
            pnodeMemberName = m_ast.CreateStrNode(memberPid);
        }
...
ParseNodePtr Parser::ParseStringTemplateDecl(ParseNodePtr pnodeTagFnc)
{
    ...
    while (!templateClosed)
    {
        ...
        if (buildAST)
        {
            stringLiteral = m_ast.CreateStrNode(m_token.GetStr());
            ...
            if (isTagged)
            {
                ...
                stringLiteralRaw = m_ast.CreateStrNode(pid);
    ...
    if (m_phtbl->TokIsUnop(m_token.tk, &opl, &nop) && nop != knopNone)
    {
        ...
        if (nop == knopYield)
        {
            if (!ParseOptionalExpr<buildAST>(&pnodeT, false, opl, NULL, TRUE))
            {
                ...
                if (buildAST)
                {
                    pnode = m_ast.CreateNode<knopYieldLeaf>(ichMin, m_pscan->IchLimTok());
                }
            }
        }
        else
...
ParseNodePtr Parser::ParseTryCatchFinally()
{
    ...
    if (tkCATCH == m_token.tk)
    {
        ...
        if (buildAST)
        {
            pnodeTC = m_ast.CreateNode<knopTryCatch>();
...
ParseNodePtr Parser::ParseFinally()
{
    ...
    if (buildAST)
    {
        ...
        if (!pnode->sxFinally.pnodeBody)
            ...
            pnode->sxFinally.pnodeBody = m_ast.CreateNode<knopEmpty>();
        else
...
ParseNodePtr Parser::ParseCatch()
{
    ...
    if (tkCATCH == m_token.tk)
    {
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopCatch>(ichMin);
...
ParseNodePtr Parser::ParseCase(ParseNodePtr *ppnodeBody)
{
    ...
    if (buildAST)
    {
        pnodeT = m_ast.CreateNode<knopCase>(ichMinT);
...
LDefaultTokenFor:
        ...
        if (TokIsForInOrForOf())
        {
            ...
            if (buildAST)
            {
                if (isForOf)
                {
                    pnode = m_ast.CreateNode<knopForOf>(ichMin);
                }
                else
                {
                    pnode = m_ast.CreateNode<knopForIn>(ichMin);
                }
        ...
        else
        {
            ...
            if (buildAST)
            {
                pnode = m_ast.CreateNode<knopFor>(ichMin);
    ...
    case tkSWITCH:
    {
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopSwitch>(ichMin);
        }
...
LEndSwitch:
    ...
    case tkWHILE:
    {
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopWhile>(ichMin);
    ...
    case tkDO:
    {
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopDoWhile>();
        }
    ...
    case tkIF:
    {
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopIf>(ichMin);
            pnode->ichLim = m_pscan->IchLimTok();
    ...
    case tkWITH:
    {
        ...
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopWith>(ichMin);
        }
    ...
    case tkBREAK:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopBreak>();
        }
        fnop = fnopBreak;
...
LGetJumpStatement:
    ...
    case tkRETURN:
    {
        if (buildAST)
        {
            ...
            pnode = m_ast.CreateNode<knopReturn>();
        }
        m_pscan->Scan();
        ParseNodePtr pnodeExpr = nullptr;
    ...
    case tkDEBUGGER:
        if (buildAST)
        {
            pnode = m_ast.CreateNode<knopDebugger>();
        }
        m_pscan->Scan();
        goto LNeedTerminator;
...
LNeedTerminator:
    ...
    if (buildAST)
    {
        ...
        if(Js::Configuration::Global.flags.SwallowExceptions)
        {
            ...
            StmtNest stmtCatch;
            ParseNodePtr pCatch;
            pCatch = m_ast.CreateNode<knopCatch>();
...
ParseNodePtr Parser::Parse(LPCUTF8 pszSrc, size_t offset, size_t length, charcount_t charOffset, ULONG grfscr, ULONG lineNumber, Js::LocalFunctionId * nextFunctionId, CompileScriptException *pse)
{
    ...
    pnodeProg = m_ast.CreateProgNode(isModuleSource);
...
ParseNode* Parser::CopyPnode(ParseNode *pnode) {
  ...
  case knopThis:
    return m_ast.CreateNode<knopThis>(pnode->ichMin,pnode->ichLim);
  ...
  case knopFalse:
    {
      ParseNode* ret = m_ast.CreateNode<knopFalse>(pnode->ichMin, pnode->ichLim);
  ...
  case knopTrue:
    {
        ParseNode* ret = m_ast.CreateNode<knopTrue>(pnode->ichMin, pnode->ichLim);
        ret->location = pnode->location;
        return ret;
    }
      //PTNODE(knopEmpty      , "empty"        ,Empty   ,None ,fnopLeaf)
  case knopEmpty:
    return m_ast.CreateNode<knopEmpty>(pnode->ichMin,pnode->ichLim);
      // Unary operators.
  ...
  case knopQmark:
    ...
    case knopVarDecl: {
      ParseNode* copyNode=m_ast.CreateNode<knopVarDecl>(pnode->ichMin,pnode->ichLim);
  ...
  case knopDebugger:
    ...
    case knopFor: {
      ParseNode* copyNode=m_ast.CreateNode<knopFor>(pnode->ichMin,pnode->ichLim);
  ...
  case knopReturn: {
    ParseNode* copyNode=m_ast.CreateNode<knopReturn>(pnode->ichMin,pnode->ichLim);
...
ParseNodePtr Parser::ParseSuper(ParseNodePtr pnode, bool fAllowCall)
{
    ...
    if (buildAST) {
        pnode = m_ast.CreateNode<knopSuper>();
    }
    ...
    BOOL *nativeForOkay)
{
    ...
    if (buildAST)
    {
        ...
        pnodeDestructAsg = m_ast.CreateNode<knopAsg>();
...
ParseNodePtr Parser::ParseDestructuredVarDecl(tokens declarationType, bool isDecl, bool *hasSeenRest, bool topLevel/* = true*/, bool allowEmptyExpression/* = true*/)
{
    ...
    if (IsPossiblePatternStart())
    {
        ...
        if (!isDecl)
        {
            ...
            pnodeElem = ParsePostfixOperators<buildAST>(pnodeElem, TRUE, FALSE, &fCanAssign, &pidTermId);
        }
    }
    ...
    {
        ParseNodePtr pnodeRest = m_ast.CreateNode<knopEllipsis>();
...
ParseNodePtr Parser::ParseDestructuredArrayLiteral(tokens declarationType, bool isDecl, bool topLevel)
{
    ...
    if (m_token.tk != tkRBrack)
    {
        while (true)
        {
            ...
            if (buildAST)
            {
                ...
                {
                    pnodeElem = m_ast.CreateNode<knopEmpty>();
                    hasMissingValues = true;
                }
    ...
    if (buildAST)
    {
        pnodeDestructArr = m_ast.CreateNode<knopArrayPattern>();
